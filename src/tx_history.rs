...
impl TxHistory {
    ...
    pub fn from_entries(entries: impl Iterator<Item=tx_result::TxEntry>,
                        now: i64) -> Self {
                ...
                .map(|(i, output)| {
                    HistoricTxOutput {
                        ...
                        value_token_base: match &entry.slp {
                            Some(slp) => slp.detail.outputs.get(i - 1).map(
...
impl TradeOffer {
    ...
    fn _decode_price(slp_decimals: i32, power_b64: &str, price_b64: &str) -> Option<_Price> {
        ...
        let script_price = price_bytes.read_u32::<LittleEndian>().ok()?;
    ...
    pub fn from_entry(tx: &HistoricTx, entry: &tx_result::TxEntry, config: &SLPDEXConfig)
        ...
        entry.inputs.iter().enumerate().find_map(|(i, input)| {
            if input.b0 == tx_result::StackItem::Str(base64::encode("EXCH")) &&
                ...
                let (token_id, token_type, slp_type) = match &tx.tx_type {
                    TxType::SLP {token_id, version, slp_type} => (token_id, *version, slp_type),
                ...
                let contract_vals = tx.outputs.get(output_idx as usize)
                    .and_then(|output: &HistoricTxOutput| {
                        ...
                        if address.addr_type() == cashcontracts::AddressType::P2SH {
